...
def save_order_payment_result():
    """保存订单支付结果"""
    alipay_dict = request.form.to_dict()  # form对象转换为字典

    # 对支付宝的数据进行分离  提取出支付宝的签名参数sign 和剩下的其他数据
    alipay_sign = alipay_dict.pop("sign")
    # alipay_sign = alipay_dict.get("sign")

    # alipay_sign = urllib.unquote(alipay_sign)
    # alipay_sign_dic = {"":alipay_sign}
    # alipay_sign1 = urllib.urlencode(alipay_sign_dic)
    # alipay_sign2 = decodestring(alipay_sign1.replace("=","").encode("utf8"))
    print("*********"*30)
    print(alipay_sign)
